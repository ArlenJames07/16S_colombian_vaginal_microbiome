...
If large files were accidentally added to Git, remove them from tracking while keeping them locally:

```bash
git rm -r --cached results
git rm --cached data/input_files/*.fastq.gz
git add .gitignore results/.gitkeep
git commit -m "Stop tracking raw data and QIIME outputs"
```
